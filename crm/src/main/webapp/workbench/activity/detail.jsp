...
<script type="text/javascript">
               ...
               $.ajax({
                   ...
                   success:function(data){
                       ...
                       if (data.success) {
                           ...
                           html +='<a class="myHref" href="javascript:void(0);" onclick="editRemark(\''+data.ar.id+'\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #ff8890;"></span></a>';
                           html +='&nbsp;&nbsp;&nbsp;&nbsp;';
                           html +='<a class="myHref" href="javascript:void(0);" onclick="deleteRemark(\''+data.ar.id+'\')"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #ff6a6e;"></span></a>';
                           html +='</div>';
                           html +='</div>';
                           html +='</div>';
                ...
                success:function(data){
                    ...
                    */
                    if(data.success){
                        ...
                        //修改备注成功
                        //更新div中相应的信息，需要更新的内容又 noteContent，editTime，editBy
	...
	function showRemarkList(){
		...
		$.ajax({
			...
			url:"workbench/activity/getRemarkListByActivity.do",
			data:{
				"activityId":"${a.id}"
			},
			type:"get",
			...
			html +='<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
			html +='<a class="myHref" href="javascript:void(0);" onclick="editRemark(\''+n.id+'\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #ff8890;"></span></a>';
			html +='&nbsp;&nbsp;&nbsp;&nbsp;';
			html +='<a class="myHref" href="javascript:void(0);" onclick="deleteRemark(\''+n.id+'\')"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #ff6a6e;"></span></a>';
			html +='</div>';
			html +='</div>';
			html +='</div>';

			})
			//用html（）方法会吧div中原有的给干掉，所以用append（）方法或者before（）方法添加
				$("#remarkDiv").before(html);
			...
			$.ajax({
				...
				success:function(data){
				    ...
				    if(data.success){
				    	...
				    	//删除成功
				    	//记录使用的是before方法，每次删除之后，页面上都会保留原有的数据
				    	//showRemarkList();
