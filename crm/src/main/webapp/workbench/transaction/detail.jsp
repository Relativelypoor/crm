...
<script type="text/javascript">
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data.success){
					...
					//改变阶段成功后，
					//(1)需要在详细信息页上局部刷新， 刷新阶段，可能性，修改人和修改时间
	...
	function changeIcon(stage,index1){
		...
		//如果当前阶段的可能性为0前7个一定是黑圈，后两个一个是红叉，一个是黑叉
		...
		<%
			...
			if("0".equals(currentPossibility)){
				...
				for(int i=0; i< dvList.size(); i++){
					...
					//取得每一个遍历出来的阶段，根据每一个遍历出来的阶段取得可能性
					DicValue dv = dvList.get(i);
			...
			//如果当前阶段的可能性不为0 前7个有可能是绿圈，绿标记，黑圈，后两个一定是黑叉
			}else{
				...
				for(int i=0; i< dvList.size(); i++) {
					...
					//如果遍历出来的阶段可能性不为0说明前7个阶段 绿圈 绿色标记 黑圈
					}else{
